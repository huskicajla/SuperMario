...
public class Pipe extends Components {
            ...
            switch (direction) {
                case Up:
                    if((KeyboardListener.isKeyPressed(GLFW_KEY_DOWN)
                            || KeyboardListener.isKeyPressed(GLFW_KEY_S))
                            && isEntrance
                            && playerAtEntrance()) {
                        playerEntering = true;
                    }
                ...
                case Left:
                    if((KeyboardListener.isKeyPressed(GLFW_KEY_RIGHT)
                            || KeyboardListener.isKeyPressed(GLFW_KEY_D))
                            && isEntrance
                            && playerAtEntrance()) {
                        playerEntering = true;
                    }
                ...
                case Down:
                    if((KeyboardListener.isKeyPressed(GLFW_KEY_UP)
                            || KeyboardListener.isKeyPressed(GLFW_KEY_W))
                            && isEntrance
                            && playerAtEntrance()) {
                        playerEntering = true;
                    }
                ...
                case Right:
                    if((KeyboardListener.isKeyPressed(GLFW_KEY_LEFT)
                            || KeyboardListener.isKeyPressed(GLFW_KEY_A))
                            && isEntrance
                            && playerAtEntrance()) {
                        playerEntering = true;
                    }
    ...
    @Override
    public void endCollision(GameObject gameObject, Contact contact, Vector2f contactNormal){
        PlayerController playerController = gameObject.getComponent(PlayerController.class);
        if(playerController != null) {
